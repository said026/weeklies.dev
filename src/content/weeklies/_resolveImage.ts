...
const allImages = import.meta.glob<{ default: ImageMetadata }>(
	"/src/content/weeklies/_images/*.{png,jpg,jpeg,webp}",
)

export async function resolveImage(entry: CollectionEntry<"weeklies">) {
	if (!(entry.data.image in allImages)) {
		throw new Error(
			`[weeklies] Image for "${entry.data.title}" not found! Provided: "${entry.data.image}", is there a typo?`,
		)
	}
...
export async function resolveAllImages(entry: CollectionEntry<"weeklies">) {
